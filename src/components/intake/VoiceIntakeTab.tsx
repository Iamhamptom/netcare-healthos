...
export default function VoiceIntakeTab({ selectedPatient, patients }: Props) {
  const [stage, setStage] = useState<"idle" | "recording" | "transcribing" | "analyzing" | "review">("idle");
  ...
  const recorder = useAudioRecorder({ chunkInterval: 2000 });

  // Load past recordings
  useEffect(() => {
    fetch("/api/intake/recordings")
      .then(r => r.json())
      .then(data => {
        if (Array.isArray(data.recordings)) setPastRecordings(data.recordings);
      })
      .catch(() => {});
  }, []);
  ...
  const handleStopAndProcess = useCallback(async () => {
    ...
    try {
      ...
      // Step 3: Save recording in background
      const patientName = patients.find(p => p.id === selectedPatient)?.name || "Unknown";
      const recFormData = new FormData();
      recFormData.append("audio", blob, "intake.webm");
      recFormData.append("duration", String(recorder.duration));
      recFormData.append("patientName", patientName);
      recFormData.append("transcriptPreview", text.slice(0, 200));
      fetch("/api/intake/recordings", { method: "POST", body: recFormData }).catch(() => {});

    } catch (err) {
      setError(err instanceof Error ? err.message : "Processing failed");
      setStage(transcript ? "review" : "idle");
    }
  }, [recorder, selectedPatient, transcript, patients]);
  ...
  const handleExport = useCallback((format: "pdf" | "fhir") => {
    if (format === "pdf" && analysis) {
      try {
        sessionStorage.setItem("intake-print-data", JSON.stringify({
          analysis,
          transcript,
          patientName: patients.find(p => p.id === selectedPatient)?.name || "Unknown Patient",
          date: new Date().toISOString(),
        }));
        window.open("/dashboard/intake/print", "_blank");
      } catch {
  ...
  }, [analysis, transcript, patients, selectedPatient]);
  ...
  const handlePlayRecording = useCallback((rec: Recording) => {
    ...
    const audio = new Audio(`/uploads/recordings/${rec.filename}`);
    ...
    <div className="space-y-6">
      ...
      <AnimatePresence>
        {error && (
          ...
          >
            <div className="w-5 h-5 text-red-400 shrink-0">!</div>
            <div>
      ...
      <div className="border border-zinc-800 rounded-xl overflow-hidden">
        ...
        >
          ...
          <ChevronDown className={`w-4 h-4 text-white/30 transition-transform ${showRecordings ? "" : "-rotate-90"}`} />
        </button>
        {showRecordings && (
          <div className="px-5 pb-4 pt-1 space-y-2">
            {pastRecordings.length === 0 ? (
              <p className="text-xs text-white/30 py-4 text-center">No recordings yet</p>
            ) : (
              pastRecordings.map((rec) => (
                ...
                >
                  <button
                    ...
                    className="w-8 h-8 rounded-full bg-teal-500/10 flex items-center justify-center shrink-0 hover:bg-teal-500/20 transition-colors"
                  >
                    {playingId === rec.id ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <span className="text-xs font-medium text-white/70">{rec.patientName}</span>
                      <span className="text-[10px] text-white/30">{formatDuration(rec.duration)}</span>
                    </div>
                    <p className="text-[11px] text-white/30 truncate">{rec.transcriptPreview || "No transcript"}</p>
                  </div>
                  <span className="text-[10px] text-white/20 shrink-0">
                    {new Date(rec.createdAt).toLocaleDateString("en-ZA")}
                  </span>
                </div>
              ))
            )}
