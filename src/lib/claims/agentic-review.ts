...
async function reasonAboutClaim(
  ...
  const prompt = "You are a senior SA medical claims reviewer. Review this claim.\n\nCLAIM:\n" + claimContext + "\n\nRULE ENGINE FLAGS:\n" + issueList + "\n\nRULE ENGINE VERDICT: " + ruleVerdict + "\n\nIMPORTANT CONSTRAINTS:\n- You can DOWNGRADE flags (REJECTED→WARNING, WARNING→VALID) if the flag is clearly wrong\n- You should be CONSERVATIVE about overriding REJECTED verdicts. Only override if you are >90% confident the rejection is wrong.\n- NEVER override: missing ICD-10, invalid format, gender mismatch, age mismatch, fabricated NAPPI, future dates, exact duplicates\n- CAN override: GP tariff scope (014 prefix can bill 0401,0407,3948,4025,4518-4537,5101), pre-auth not needed for GP X-rays, clinical motivation explains flag\n- Injury codes (S/T) without external cause (V/W/X/Y) is a REAL error — do NOT override\n- Tariff 0199 = chronic repeat script, valid for all ages\n- Dependent code is sequence number, not age\n- R05 (cough) as primary is acceptable for acute undifferentiated presentation\n\nRespond in JSON:\n{\"verdict\": \"VALID\" or \"WARNING\" or \"REJECTED\", \"reasoning\": \"2-3 sentences\", \"confidence\": 0.0-1.0, \"suggestions\": []}";
...
export async function runAgenticReview(
  ...
  for (let i = 0; i < flaggedClaims.length; i += 5) {
    ...
    for (let j = 0; j < batch.length; j++) {
      ...
      // SAFETY: If rule engine said REJECTED and AI says VALID, require >90% confidence
      let finalVerdict = ai.verdict;
      if (ruleVerdict === "REJECTED" && ai.verdict === "VALID" && ai.confidence < 0.9) {
        finalVerdict = "WARNING"; // Downgrade to WARNING instead of full override
        ai.reasoning += " [SAFETY: AI confidence " + (ai.confidence * 100).toFixed(0) + "% < 90% threshold for rejection override — downgraded to WARNING]";
      }
