...
export const MODULE_DEFINITIONS: ModuleDefinition[] = [
  ...
  /* ────────────────────────────────────────────────────────────── */
  {
    ...
    name: "Front Desk",
    shortName: "Reception",
    description: "Everything reception needs in one place — bookings, check-in, eligibility verification, patient engagement, notifications, daily tasks, and a dedicated AI agent.",
    icon: "Landmark",
    color: "#2DD4BF",
    badge: "NEW",
    badgeColor: "emerald",
    roles: ["admin", "receptionist", "nurse"],
    pages: [
      { href: "/dashboard/front-desk", icon: "Landmark", label: "Front Desk Hub", roles: ["admin", "receptionist", "nurse"], step: 1 },
      { href: "/dashboard/bookings", icon: "CalendarCheck", label: "Bookings", roles: ["admin", "receptionist"], step: 2 },
      { href: "/dashboard/checkin", icon: "UserCheck", label: "Check-In Queue", roles: ["admin", "receptionist", "nurse"], step: 3 },
      { href: "/dashboard/calendar", icon: "Calendar", label: "Schedule", roles: ["admin", "receptionist"], step: 4 },
      { href: "/dashboard/daily", icon: "ClipboardList", label: "Daily Tasks", roles: ["admin", "receptionist", "nurse"], step: 5 },
      { href: "/dashboard/front-desk/engagement", icon: "HeartPulse", label: "Patient Care", roles: ["admin", "receptionist", "nurse"], step: 6 },
      { href: "/dashboard/front-desk/connections", icon: "PlugZap", label: "Connections", roles: ["admin", "receptionist"], step: 7 },
      { href: "/dashboard/recall", icon: "RotateCcw", label: "Recall", roles: ["admin", "receptionist"], step: 8 },
      { href: "/dashboard/notifications", icon: "Bell", label: "Notifications", roles: ["admin", "receptionist"], step: 9 },
      { href: "/dashboard/approvals", icon: "CheckCircle2", label: "Approvals", roles: ["admin", "receptionist"], step: 10 },
    ],
    integrations: [
      {
        id: "supabase",
        ...
        status: "connected",
        impact: "Everything breaks — no data storage at all",
        fallback: "None — required",
        envKeys: ["DATABASE_URL", "NEXT_PUBLIC_SUPABASE_URL"],
      },
      {
        ...
        description: "Appointment confirmations, reminders, patient chatbot, engagement sequences",
        status: "needs_setup",
        impact: "No WhatsApp notifications — falls back to email via Resend",
        fallback: "Email via Resend for all notifications",
        envKeys: ["TWILIO_ACCOUNT_SID", "TWILIO_AUTH_TOKEN"],
        setupHint: "Each clinic needs a WhatsApp Business account linked to Twilio + approved message templates.",
      },
      {
        id: "resend",
        name: "Email (Resend)",
        description: "Booking confirmations, reminders, follow-ups, review requests",
        status: "connected",
        impact: "No email delivery — WhatsApp becomes the only channel",
        fallback: "WhatsApp only (if connected), or manual phone calls",
        envKeys: ["RESEND_API_KEY"],
        setupHint: "Domain visiocorp.co is verified. DKIM/SPF green.",
      },
      {
        id: "google-calendar",
        name: "Google Calendar",
        ...
        status: "needs_setup",
        impact: "No calendar sync — bookings only visible in Health OS dashboard",
        fallback: "In-app calendar only",
        envKeys: ["GOOGLE_CALENDAR_CLIENT_ID", "GOOGLE_CALENDAR_CLIENT_SECRET"],
        setupHint: "Requires Google OAuth consent screen + API credentials.",
      },
      {
        id: "heal",
        name: "HEAL System (A2D24)",
        description: "Import bookings from South Africa's most-used practice management system",
        status: "stubbed",
        impact: "Bookings made in HEAL don't appear — reception manually enters or does CSV import",
        fallback: "CSV import at /dashboard/import, or manual entry",
        setupHint: "HEAL API access not yet available. CSV import is the current workaround.",
      },
      {
        id: "healthbridge",
        name: "Healthbridge / MediKredit",
        description: "Real-time medical aid eligibility verification before consultation",
        status: "needs_setup",
        impact: "No real-time eligibility check — reception verifies manually",
        fallback: "Phone medical aid or check membership card manually",
        envKeys: ["HEALTHBRIDGE_API_KEY", "HEALTHBRIDGE_URL"],
        setupHint: "Practice must be registered with Healthbridge. Credentials from practice accreditation.",
      },
      {
        id: "ai-model",
        name: "AI Models (Claude / Gemini)",
        description: "Powers the Front Desk Agent, patient chatbot, triage, and side effects check",
        status: "connected",
        impact: "AI features go offline — reception answers questions manually",
        fallback: "Manual operations. Engagement sequences still send but without AI triage.",
        envKeys: ["ANTHROPIC_API_KEY", "GOOGLE_GENERATIVE_AI_API_KEY"],
      },
    ],
    agentContext: {
      scope: "Front desk operations — bookings, check-in, eligibility, notifications, daily tasks, patient engagement",
      capabilities: [
        "Create, confirm, reschedule, and cancel bookings",
        "Check patients in and move through Kanban (waiting → in consult → done)",
      ...
      priorityTools: ["create_booking", "checkin_patient", "verify_medical_aid", "send_notification", "list_daily_tasks", "search_patients"],
    },
  },
];
