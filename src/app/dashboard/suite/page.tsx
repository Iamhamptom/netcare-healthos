...
import {
  ...
  Smartphone, FileText, TrendingUp, Clock, ArrowLeftRight, Network,
} from "lucide-react";
...
const VALUE_CHAIN = [
  {
    ...
    items: [
      { name: "Medicross Clinics (55)", icon: Building2, desc: "GP, Dental, Pharmacy, Optometry — full suite" },
      { name: "Prime Cure Managed Care", icon: Heart, desc: "Capitation analytics, occupational health billing" },
      { name: "Netcare Pharmacies (41)", icon: Pill, desc: "Inventory intelligence, NAPPI validation, CDL monitoring" },
      { name: "Day Theatres (12)", icon: Stethoscope, desc: "Procedure billing, theatre scheduling, claims" },
      { name: "Netcare 911", icon: Ambulance, desc: "Emergency triage routing, WhatsApp 911 escalation" },
      { name: "National Renal Care", icon: Heart, desc: "Chronic dialysis billing, scheme benefit tracking" },
    ],
  },
  {
    category: "Existing Systems We Integrate With",
    items: [
      { name: "CareOn EMR", icon: FileText, desc: "Hospital EMR — we extend to primary care" },
      { name: "Healthbridge / GoodX", icon: Globe, desc: "Clinic PMS — we unify the fragmented landscape" },
      { name: "Altron SwitchOn", icon: ArrowLeftRight, desc: "Claims switching — we pre-validate before submission" },
      { name: "MediSwitch EDI", icon: Network, desc: "eRA processing — we auto-reconcile payments" },
      { name: "IBM Watson Micromedex", icon: Shield, desc: "Drug interactions — we surface alerts in booking flow" },
      { name: "Netcare App", icon: Smartphone, desc: "Patient portal — we power the backend intelligence" },
    ],
  },
  {
    category: "What We Replace / Upgrade",
    items: [
      { name: "Manual Excel Reporting", icon: FileText, desc: "→ Real-time divisional dashboards" },
      { name: "Phone-Based Booking", icon: Phone, desc: "→ WhatsApp + Online self-service booking" },
      { name: "Manual Claims Follow-up", icon: Clock, desc: "→ AI pre-validation + auto-resubmission" },
      { name: "Per-Clinic Compliance Audits", icon: Shield, desc: "→ Automated POPIA compliance engine" },
      { name: "Fragmented Patient Records", icon: Users, desc: "→ Unified cross-site patient view" },
      { name: "Manual Recall Management", icon: Target, desc: "→ AI-powered automated recall sequences" },
    ],
  },
];
