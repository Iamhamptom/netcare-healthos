...
function DashboardLayoutInner({ children }: { children: React.ReactNode }) {
  ...
  // Full-screen pages — no sidebar, no header, no chrome
  const [pathname, setPathname] = useState("");
  useEffect(() => { setPathname(window.location.pathname); }, []);
  const isFullScreen = pathname === "/dashboard/home" || pathname === "/dashboard/pitch";
  ...
  return (
    <div className={`flex h-screen ${isFullScreen ? "bg-[#0a0f1a]" : "bg-[#f0f2f5]"}`}>
      <AnimatePresence>
      ...
      {/* Sidebar + Header hidden on full-screen pages (home, pitch) */}
      {!isFullScreen && (
        <>
          {mobileOpen && (
            <div
