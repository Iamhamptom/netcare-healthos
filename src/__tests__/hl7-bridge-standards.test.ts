...
describe("HL7v2.4 Ch2: Message Framework Compliance", () => {
  describe("2.6 — Message Construction Rules", () => {
    it("MSH-9 (message type) parsed as component^trigger", () => {
      const msg = parseHL7Message(mkAdmit("FAC", "MRN-1", "DISC"));
      expect(msg.messageType).toMatch(/^\w+\^\w+$/); // ADT^A01 pattern
    });

    it("MSH-10 (message control ID) is unique per message", () => {
    ...
    it("MSH-12 (version ID) defaults to 2.4", () => {
      const msg = parseHL7Message(mkAdmit("FAC", "MRN-1", "DISC"));
      expect(msg.version).toBe("2.4");
    });

    it("segments are delimited by CR (\\r)", () => {
      const raw = mkAdmit("FAC", "MRN-1", "DISC");
      expect(raw).toContain("\r");
      expect(raw).not.toContain("\r\n"); // Not CRLF
    });

    it("fields are delimited by pipe (|)", () => {
      const msg = parseHL7Message(mkAdmit("FAC", "MRN-1", "DISC"));
    ...
    it("components are delimited by caret (^)", () => {
      const msg = parseHL7Message(mkAdmit("FAC", "MRN-1", "DISC"));
  ...
  describe("2.9 — Acknowledgment Protocol", () => {
    it("ACK contains MSH + MSA segments", () => {
      const msg = parseHL7Message(mkAdmit("FAC", "MRN-1", "DISC"));
    ...
    it("MSA-1 reflects acknowledgment code (AA/AE/AR)", () => {
      const msg = parseHL7Message(mkAdmit("FAC", "MRN-1", "DISC"));
    ...
    it("MSA-2 echoes original message control ID", () => {
      const msg = parseHL7Message(mkAdmit("FAC", "MRN-1", "DISC"));
      const ack = generateACK(msg);
      expect(ack).toContain(`MSA|AA|MSG-MRN-1`);
    });

    it("ACK MSH references original sending application", () => {
      const msg = parseHL7Message(mkAdmit("FAC", "MRN-1", "DISC"));
      const ack = generateACK(msg);
  ...
  describe("Robustness — Malformed Input Handling", () => {
    ...
    it("rejects message without MSH", () => {
    ...
    it("handles message with trailing whitespace/newlines", () => {
      const raw = mkAdmit("FAC", "MRN-1", "DISC") + "\r\n\r\n  ";
      const msg = parseHL7Message(raw);
      expect(msg.messageType).toBe("ADT^A01");
    });

    it("handles message with mixed line endings", () => {
      const raw = mkAdmit("FAC", "MRN-1", "DISC").replace(/\r/g, "\r\n");
      const msg = parseHL7Message(raw);
      expect(msg.messageType).toBe("ADT^A01");
    });

    it("handles MSH with missing optional fields", () => {
    ...
    it("handles PID with minimal fields", () => {
    ...
    it("handles DG1 with empty description", () => {
...
describe("FHIR R4 4.0.1: Resource Conformance", () => {
  ...
  describe("Patient Resource (FHIR 4.0.1 §4.1)", () => {
    it("resourceType is 'Patient'", () => {
      expect(mapPatientToFHIR(patient).resourceType).toBe("Patient");
    });

    it("has at least one identifier (MRN)", () => {
  ...
  describe("Encounter Resource (FHIR 4.0.1 §4.6)", () => {
    it("resourceType is 'Encounter'", () => {
      expect(mapEncounterToFHIR(encounter, patient.id).resourceType).toBe("Encounter");
    });

    it("status is valid (planned|arrived|in-progress|finished|cancelled)", () => {
    ...
    it("class uses v3 ActEncounterCode (IMP|AMB|EMER)", () => {
      const fhir = mapEncounterToFHIR(encounter, patient.id);
      expect(["IMP", "AMB", "EMER"]).toContain(fhir.class.code);
    });

    it("subject references Patient resource", () => {
      const fhir = mapEncounterToFHIR(encounter, patient.id);
      expect(fhir.subject.reference).toMatch(/^Patient\//);
    });

    it("period.start is set (may be empty if no admit date in PV1)", () => {
      const fhir = mapEncounterToFHIR(encounter, patient.id);
      // period.start comes from PV1-44 (admitDate) — may be empty if not in segment
      expect(fhir.period.start).toBeDefined();
    });

    it("inpatient maps to IMP, outpatient to AMB, emergency to EMER", () => {
      const classes = [
        { hl7: "I", fhir: "IMP" },
        { hl7: "O", fhir: "AMB" },
        { hl7: "E", fhir: "EMER" },
...
describe("SA HNSF: Healthcare Coding Standards", () => {
  ...
  describe("LOINC Code Detection in Observations", () => {
    ...
    it("non-LOINC code uses local system", () => {
...
describe("POPIA Compliance (Sections 14, 19, 23)", () => {
  describe("Section 14 — Conditions for Processing (De-identification)", () => {
    ...
    it("receptionist sees limited data (purpose limitation)", () => {
    ...
    it("unknown roles see fully masked data", () => {
      const level = getDeidentLevel("external");
      expect(level.showPatientName).toBe(false);
      expect(level.showIdNumber).toBe(false);
    });

    it("advisory de-identification masks MRN digits for full level", () => {
  ...
  describe("Section 19 — Security Safeguards (Audit Trail)", () => {
    ...
    it("audit entries contain required POPIA fields", () => {
      logBridgeAudit({
        action: "view_patient",
        userId: "usr-001",
        userName: "Dr Smith",
    ...
    it("audit log maintains order (newest first)", () => {
  ...
  describe("Facility-Scoped Access (Data Minimization)", () => {
    it("restricts non-admin to assigned facilities", () => {
    ...
    it("platform_admin bypasses facility restriction", () => {
...
describe("OWASP Top 10: Security", () => {
  describe("A01:2021 — Broken Access Control", () => {
    it("facility scoping prevents cross-facility data access", () => {
      expect(hasAccessToFacility(["sandton"], "Netcare Milpark Hospital", "receptionist")).toBe(false);
    });

    it("role escalation is prevented (receptionist cannot become admin)", () => {
  ...
  describe("A02:2021 — Cryptographic Failures", () => {
    it("HMAC-SHA256 verification rejects wrong signatures", () => {
      expect(verifyWebhookSignature("payload", "wrong-hex", "secret")).toBe(false);
    });

    it("HMAC-SHA256 accepts correct signatures", () => {
    ...
    it("null signature is rejected", () => {
      expect(verifyWebhookSignature("payload", null, "secret")).toBe(false);
    });
  });

  describe("A03:2021 — Injection", () => {
    it("HL7 parser handles injection in patient name", () => {
    ...
    it("HL7 parser handles SQL injection in fields", () => {
  ...
  describe("A07:2021 — Identification and Authentication Failures", () => {
    it("webhook auth validates request headers structure", () => {
...
describe("Bridge Advisory Engine: End-to-End", () => {
  describe("Advisory Generation by Message Type", () => {
    it("ADT^A01 (admit) generates billing + eligibility advisories", () => {
    ...
    it("ORU^R01 with critical flag generates clinical advisory", () => {
    ...
    it("ORU^R01 with normal flag generates no clinical advisory", () => {
      const result = processHL7Message(mkLab("718-7", "13.2", "g/dL", "N"));
      const clinical = result.advisories.find(a => a.category === "clinical");
      expect(clinical).toBeUndefined();
    });

    it("advisories include estimated claim value", () => {
    ...
    it("inpatient claims have higher estimated value than outpatient", () => {
      // Inpatient
  ...
  describe("Advisory Resolution Workflow", () => {
    it("all resolution actions produce valid audit trail", () => {
      ...
      for (const action of actions) {
        ...
        const result = resolveAdvisory(unresolved.id, action, `Test-${action}`);
    ...
    it("generate_claim produces CLM- prefixed draft ID", () => {
      processHL7Message(mkAdmit("FAC", "MRN-CLM", "DISC"));
      const adv = getAdvisories({ limit: 1 }).find(a => !a.resolution && a.actionRequired);
      if (!adv) return;
      const result = resolveAdvisory(adv.id, "generate_claim", "Tester");
      expect(result.advisory?.resolution?.claimDraftId).toMatch(/^CLM-/);
    });

    it("notify_doctor sets notificationSent flag", () => {
      processHL7Message(mkAdmit("FAC", "MRN-NOT", "DISC"));
      const adv = getAdvisories({ limit: 1 }).find(a => !a.resolution && a.actionRequired);
      if (!adv) return;
      const result = resolveAdvisory(adv.id, "notify_doctor", "Tester");
  ...
  describe("Bridge Stats Integrity", () => {
    ...
    it("claim value is non-negative", () => {
      const stats = getBridgeStats();
...
describe("Demo Data: Production Quality", () => {
  it("all 5 demo HL7 messages parse and extract patients", () => {
  ...
  it("all demo advisories have valid severity levels", () => {
  ...
  it("all demo advisories have valid categories", () => {
  ...
  it("all demo ICD-10 suggestions have confidence between 0 and 1", () => {
    for (const adv of DEMO_ADVISORIES) {
  ...
  it("all demo advisories have non-negative estimated values", () => {
    for (const adv of DEMO_ADVISORIES) {
